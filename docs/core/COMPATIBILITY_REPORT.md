...
**ขอบเขต:** nakhara-monolith (Rust Core + Python DeAI + ops + configs + scripts)
...
| **ops/deploy validator scripts** | ✅ แก้แล้ว | ใช้ nakhara-monolith และ path core/deai แล้ว |
...
- **pyo3:** workspace.dependencies กำหนด pyo3 = "0.22"; crate `nakhara-python` ใช้ 0.24 เพื่อ RUSTSEC-2025-0020 — เป็นการ override โดยตั้งใจ
...
สคริปต์ใน **ops/deploy/** อัปเดตให้ใช้ repo **nakhara-monolith** และ path ตรงกับโครงสร้างแล้ว:

| ไฟล์ | การแก้ไข |
|------|----------|
| `ops/deploy/setup_validator.sh` | REPO_URL → nakhara-monolith.git, REPO_DIR, build จาก core/, DeAI ที่ core/deai, PYTHONPATH และ path ในขั้นตอนถัดไป |
| `ops/deploy/setup_systemd.sh` | WorkingDirectory → nakhara-monolith, PYTHONPATH → .../core/deai |
| `ops/deploy/setup_rpc_node.sh` | NAKHARA_HOME → nakhara-monolith, clone repo นี้, build จาก core/, binary nakhara-core, ExecStart → /usr/local/bin/nakhara-core |
| `core/tools/GENESIS_LAUNCH_README.md` | ชี้ setup_systemd.sh ไปที่ ~/nakhara-monolith/services/core/ops/deploy/scripts/setup_systemd.sh |

หมายเหตุ: ชื่อ binary **nakhara-core** ใช้ใน ExecStart และการติดตั้ง — สอดคล้องกับ crate name ใน core/
...
2. **ถ้าใช้ ops/deploy บน VPS:** แก้ setup_validator.sh, setup_systemd.sh, setup_rpc_node.sh ให้ใช้ repo **nakhara-monolith** และ path **core/deai**, **scripts/** ตามโครงสร้างนี้
