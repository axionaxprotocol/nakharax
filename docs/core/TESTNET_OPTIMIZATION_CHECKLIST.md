...
- [ ] **RPC proxy ขึ้น** — `https://rpc.nakharax.com` ตอบ
  ```bash
...
- [ ] **CORS ถูกต้อง** — frontend (nakharax.com) เรียก RPC ได้ไม่มี CORS error
- [ ] **HTTPS ใช้งานได้** — ใช้ `https://rpc.nakharax.com` ไม่ fallback เป็น HTTP
...
- [ ] **Faucet ขึ้น** — `https://faucet.nakharax.com` ตอบ (ตาม [VPS_AU_ALL_IN_ONE.md](../../services/core/ops/deploy/VPS_AU_ALL_IN_ONE.md))
...
for url in http://217.216.109.5:8545 http://46.250.244.4:8545 https://rpc.nakharax.com; do
  echo -n "$url: "
  curl -s -X POST -H "Content-Type: application/json" \
