...
- [ ] **RPC proxy ขึ้น** — `https://rpc.nakharaxx.io` ตอบ
  ```bash
...
- [ ] **CORS ถูกต้อง** — frontend (nakharaxx.io) เรียก RPC ได้ไม่มี CORS error
- [ ] **HTTPS ใช้งานได้** — ใช้ `https://rpc.nakharaxx.io` ไม่ fallback เป็น HTTP
...
- [ ] **Faucet ขึ้น** — `https://faucet.nakharaxx.io` ตอบ (ตาม [VPS_AU_ALL_IN_ONE.md](../../services/core/ops/deploy/VPS_AU_ALL_IN_ONE.md))
...
for url in http://217.216.109.5:8545 http://46.250.244.4:8545 https://rpc.nakharaxx.io; do
  echo -n "$url: "
  curl -s -X POST -H "Content-Type: application/json" \
