...
- [ ] **Faucet key** — AU (`46.250.244.4`) ตั้ง `FAUCET_PRIVATE_KEY` ใน `.env` ให้ตรงกับ genesis (deterministic จาก seed `nakhara_faucet_mainnet_q2_2026` ถ้าใช้ default)
...
```bash
scp core/tools/genesis.json root@217.216.109.5:~/.nakhara/config/
scp core/tools/genesis.json root@46.250.244.4:~/.nakhara/config/
# บนแต่ละ VPS: sha256sum ~/.nakhara/config/genesis.json ต้องตรงกับ 0xed1bdac7...
```
...
ตรวจว่า config ใช้ **chain_id 86137** และ node อ่าน genesis จาก `~/.nakhara/config/genesis.json`
...
# ดู log ของ node (แทน <CONTAINER> ด้วยชื่อจากด้านบน เช่น nakhara-rpc หรือ rpc-node)
docker logs <CONTAINER> --tail 50

# ตรวจ config ว่า chain_id 86137 (path ขึ้นกับที่ deploy — มักอยู่ข้าง docker-compose)
grep -E "chain_id|genesis" /opt/nakhara-deploy/configs/rpc-config.toml 2>/dev/null || \
grep -E "chain_id|genesis" ~/nakhara-monolith/services/core/ops/deploy/configs/rpc-config.toml 2>/dev/null || \
...
- [ ] Frontend ตั้ง `NEXT_PUBLIC_RPC_URL=https://rpc.nakhara.io` และทดสอบ Add Network (86137) + รับ AXX จาก Faucet
...
ssh root@217.216.109.5 'bash /tmp/fix-validator-ulimit.sh nakhara-validator-eu'
```
...
bash ops/deploy/scripts/fix-validator-ulimit.sh nakhara-validator-eu
```
...
Script จะ recreate container ด้วย `--ulimit nofile=65536:65536` อัตโนมัติ หลังรันตรวจ: `docker logs nakhara-validator-eu --tail 20`
