...
|-----------|----------------|-------------|
| **VPS EU** | 217.216.109.5 | Validator #1, RPC, **Nakharax OS** (`app.nakharax.io` → :3030) |
| **VPS AU** | 46.250.244.4 | Validator #2; nginx, rpc, explorer, api, faucet |
| **DNS (chain)** | rpc / explorer / api / faucet `.nakharax.io` | → **46.250.244.4** |
| **DNS (OS)** | `app.nakharax.io` | → **217.216.109.5** |
| **Nakharax OS** | `apps/os-dashboard` on EU | RPC client → `https://rpc.nakharax.io` (AU) |
...
```
[User] → https://rpc.nakharax.io (nginx @ 46.250.244.4)
              ↓
...
- **Frontend** should use `NEXT_PUBLIC_RPC_URL=https://rpc.nakharax.io` (or `https://rpc-au.nakharax.io`).
...
| Frontend RPC env | build-time | `https://rpc.nakharax.io` |
...
- Production: `https://rpc.nakharax.io` → testnet 86137 on AU/EU validators.
...
| **Frontend ↔ testnet** | Yes | RPC URL points to rpc.nakharax.io (AU nginx → local node) |
