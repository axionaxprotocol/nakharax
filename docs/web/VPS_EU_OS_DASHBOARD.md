# Deploy Axionax OS Dashboard on EU (217.216.109.5)

**Host:** `217.216.109.5` (EU)  
**App:** `apps/os-dashboard` (Next.js 14, Obsidian OS UI)  
**Suggested domain:** `https://app.axionax.org` (หรือ `https://os.axionax.org`)
...
| VPS | IP | บทบาท |
|-----|-----|--------|
| **EU** | 217.216.109.5 | Validator #1 + RPC + **Axionax OS** (port 3030) |
...
```bash
cd /opt/axionax-monolith/apps/os-dashboard
...
จาก **repo root** (`/opt/axionax-monolith`):
...
PORT=3030 pm2 start .next/standalone/apps/os-dashboard/server.js --name axionax-os
pm2 save
```
...
คัดลอก [`apps/os-dashboard/nginx/app.axionax.org.conf.example`](../../apps/os-dashboard/nginx/app.axionax.org.conf.example) ไป `/etc/nginx/sites-enabled/` แล้ว:

```bash
certbot certonly --nginx -d app.axionax.org
nginx -t && systemctl reload nginx
```

DNS: **A record** `app.axionax.org` → `217.216.109.5`
...
curl -sI https://app.axionax.org | head -1
```
