...
Lightweight discipline when you are the only person touching **nakharax-monolith** and **nakharax-monolith**.
...
2. **This repo mirrors** those values in `packages/blockchain-utils` (and anything that imports it, including `@nakharax/sdk`). See [PARAMETERS_SYNC.md](../packages/blockchain-utils/PARAMETERS_SYNC.md) for the file mapping.
...
- [ ] Align numbers/names; run `pnpm --filter @nakharax/blockchain-utils type-check` (or `pnpm --filter @nakharax/sdk test`) from the monorepo root.
...
- Core repo: https://github.com/nakharax-io/nakharax-monolith  
