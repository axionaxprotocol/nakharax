...
export default function MCPMarketplacePage() {
  ...
  const copyMcpConfig = () => {
    const config = {
      mcpServers: {
        "nakharax-quant-risk": {
          url: "https://rpc.nakharax.com/mcp/quant-risk/sse",
          transport: "sse",
        },
        "nakharax-evm-auditor": {
          url: "https://rpc.nakharax.com/mcp/auditor",
          transport: "websocket",
        },
        "nakharax-web-harvester": {
          url: "https://rpc.nakharax.com/mcp/crawler/sse",
          transport: "sse",
        },
      },
    };
    navigator.clipboard.writeText(JSON.stringify(config, null, 2));
    setExportedNotice("📋 Copied Cursor / Antigravity MCP Config to clipboard!");
    setTimeout(() => setExportedNotice(null), 5000);
  };
      ...
      actions={
        <>
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-full border border-cyan-500/30 bg-cyan-500/10 hover:bg-cyan-500/20 px-4 py-2 text-[11px] font-semibold text-cyan-300 transition-colors"
          >
            <Copy size={13} />
            Copy Cursor / IDE MCP JSON
          </button>
          <button
            type="button"
