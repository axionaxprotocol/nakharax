#!/bin/bash
# Setup monitoring and backups for nakharax testnet
...
docker exec nakharax-postgres pg_dumpall -U postgres > "$BACKUP_DIR/postgres_$DATE.sql"

# Backup Redis
echo "Backing up Redis..."
docker exec nakharax-redis redis-cli SAVE
docker cp nakharax-redis:/data/dump.rdb "$BACKUP_DIR/redis_$DATE.rdb"
...
(crontab -l 2>/dev/null | grep -v "nakharax-backup"; echo "0 2 * * * /root/backups/backup.sh >> /var/log/nakharax-backup.log 2>&1") | crontab -
