...
Set up a RunPod A40 GPU as a worker node for the nakharax testnet in 3 steps
...
```bash
# Edit worker config
nano /workspace/nakharax-worker/config/worker.toml
...
# Re-attach: tmux attach -t nakharax-worker
```
...
# View logs
tail -f /workspace/nakharax-worker/logs/worker.log
```
