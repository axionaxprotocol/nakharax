...
# =============================================================================
# nakharax Protocol - VPS Status Check Script
# =============================================================================
...
check_service_health() {
    ...
    declare -A services=(
        ...
        ["faucet"]="Faucet API|3002|[ -n \"\$(docker ps -q --filter name=nakharax-faucet --filter status=running)\" ]"
        ["prometheus"]="Prometheus|9090|[ -n \"\$(docker ps -q --filter name=nakharax-prometheus --filter status=running)\" ]"
        ["postgres"]="PostgreSQL|5432|docker exec nakharax-postgres pg_isready -U explorer"
        ["redis"]="Redis|6379|docker exec nakharax-redis redis-cli -a \"${REDIS_PASSWORD}\" ping"
    )
...
check_database() {
    ...
    if docker ps | grep -q "nakharax-postgres"; then
        print_success "PostgreSQL container is running"
        
        # Database size
        DB_SIZE=$(docker exec nakharax-postgres psql -U explorer -d explorer -t -c "SELECT pg_size_pretty(pg_database_size('explorer'));" 2>/dev/null | xargs)
        echo -e "Database Size:     ${DB_SIZE}"
        
        # Connection count
        CONNECTIONS=$(docker exec nakharax-postgres psql -U explorer -d explorer -t -c "SELECT count(*) FROM pg_stat_activity;" 2>/dev/null | xargs)
        echo -e "Active Connections: ${CONNECTIONS}"
        
        # Table count
        TABLES=$(docker exec nakharax-postgres psql -U explorer -d explorer -t -c "SELECT count(*) FROM information_schema.tables WHERE table_schema = 'public';" 2>/dev/null | xargs)
    ...
    if docker ps | grep -q "nakharax-redis"; then
        print_success "Redis container is running"
        
        # Redis memory usage
        REDIS_MEM=$(docker exec nakharax-redis redis-cli -a "${REDIS_PASSWORD}" INFO memory 2>/dev/null | grep "used_memory_human" | cut -d: -f2 | tr -d '\r')
        echo -e "Redis Memory:      ${REDIS_MEM}"
        
        # Redis keys
        REDIS_KEYS=$(docker exec nakharax-redis redis-cli -a "${REDIS_PASSWORD}" DBSIZE 2>/dev/null | awk '{print $2}')
...
main() {
    clear
    print_header "nakharax VPS Status Check"
