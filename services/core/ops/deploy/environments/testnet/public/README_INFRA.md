...
For **air-gapped / self-sufficient** operation (no dependency on external URLs), remove the two `--telemetry` lines from the **validator** service command in `docker-compose.yaml`. The node will run without sending data to telemetry.nakharax.com.
