...
cd /opt/nakhara-deploy/mock-rpc
docker build --no-cache --pull -t nakhara-mock-rpc:latest .
docker stop nakhara-rpc && docker rm nakhara-rpc
docker run -d --name nakhara-rpc \
  --network nakhara-deploy_nakhara-net \
  -p 8545:8545 -p 8546:8546 \
  -e CHAIN_ID=888 -e NETWORK=nakhara-testnet-1 \
  --restart unless-stopped \
  nakhara-mock-rpc:latest

# Option B: Test from another container (not curl)
docker exec nakhara-faucet-api wget -qO- \
  --post-data='{"jsonrpc":"2.0","method":"eth_blockNumber","params":[],"id":1}' \
  --header='Content-Type:application/json' \
  http://nakhara-rpc:8545
```

### Priority 2: Check Application Service Logs
```bash
docker logs nakhara-faucet-api --tail=50
docker logs nakhara-explorer-api --tail=50
docker logs nakhara-prometheus --tail=50
```
...
Located in `/opt/nakhara-deploy/.env`:
```bash
# Database
...
DOMAIN=<your_domain>
CHAIN_ID=888
NETWORK=nakhara-testnet-1
```

### RPC Configuration
Located in `/opt/nakhara-deploy/configs/rpc-config.toml`:
- Network ID: nakhara-testnet-1
- Chain ID: 888
...
- **Deployment Guide:** `/opt/nakhara-deploy/scripts/README.md`
- **Quick Reference:** `/opt/nakhara-deploy/scripts/QUICK_REFERENCE.md`
- **RPC Server Docs:** `/opt/nakhara-deploy/mock-rpc/README.md`
- **Connection Commands:** `/opt/nakhara-deploy/VPS_CONNECTION.txt`
...
**Deployment Scripts:** https://github.com/nakhara-io/nakhara-deploy  
