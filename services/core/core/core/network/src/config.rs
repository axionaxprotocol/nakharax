...
///   sourced from `external_addrs` or the `NAKHARAX_EXTERNAL_ADDRS` env var. Use
...
impl Default for NetworkConfig {
    fn default() -> Self {
        Self {
            ...
            chain_id: 86137, // nakharax Testnet
            key_file: None,
...
impl NetworkConfig {
    ...
    ///    *Example:* `/ip4/rpc-au.nakharaxx.io/tcp/30303,/ip4/rpc-au.nakharaxx.io/udp/30303/quic-v1`
    ...
    pub fn resolved_external_addrs(&self) -> Vec<String> {
        ...
        // 1. NAKHARAX_EXTERNAL_ADDRS — explicit multiaddr list
        if let Ok(raw) = std::env::var("NAKHARAX_EXTERNAL_ADDRS") {
        ...
        // 2. NAKHARAX_PUBLIC_IP — bare IP, we synthesize the TCP multiaddr
        if let Ok(ip) = std::env::var("NAKHARAX_PUBLIC_IP") {
            let ip = ip.trim();
            if !ip.is_empty() {
...
impl NetworkConfig {
    /// Create config for testnet (bootstrap via NAKHARAX_BOOTSTRAP_NODES env or add peers manually)
    pub fn testnet() -> Self {
        Self {
            chain_id: 86137,
            bootstrap_nodes: vec![], // Set NAKHARAX_BOOTSTRAP_NODES=/ip4/<IP>/tcp/30303/p2p/<PEER_ID> on VPS
            block_time_seconds: 5,
            ..Default::default()
        }
    }

    /// Create config for mainnet (bootstrap via NAKHARAX_BOOTSTRAP_NODES env)
    pub fn mainnet() -> Self {
        Self {
            chain_id: 86150,
            validation_mode: ValidationMode::Strict,
...
mod tests {
    ...
    fn test_testnet_config() {
        ...
        assert_eq!(config.chain_id, 86137);
        // bootstrap_nodes is empty by default; operators set NAKHARAX_BOOTSTRAP_NODES at runtime
