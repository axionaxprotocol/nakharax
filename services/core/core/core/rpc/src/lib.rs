...
///
/// Since nakharax tx execution is all-or-nothing (tx is applied atomically when
...
#[async_trait]
impl NakharaxRpcServer for NakharaxRpcServerImpl {
    async fn block_number(&self) -> RpcResult<String> {
...
pub async fn start_rpc_server(
    ...
    let cors = {
        ...
        let allowed_origins = std::env::var("NAKHARAX_RPC_CORS_ORIGINS").unwrap_or_default();
    ...
    let rate_limit_rps: u64 = std::env::var("NAKHARAX_RPC_RATE_LIMIT")
        .ok()
    ...
    let mut rpc_impl = NakharaxRpcServerImpl::new(state.clone(), chain_id);
