...
2. **Set the contract address** in one of:
   - **Config:** In `worker_config.toml` (or your TOML), under `[network]`:
     ...
     ```
   - **Environment:** `NAKHARAX_MARKETPLACE_ADDRESS=0xYourDeployedContractAddress`
3. **ABI:** The worker loads the ABI from `core/deai/job_marketplace.json`. If you use a different path, set it (or patch the code to use `NAKHARAX_ABI_PATH`).
4. **Wallet:** Worker must have NAK for gas and (if required) stake. Use `NAKHARAX_WALLET_PATH` / `WORKER_PRIVATE_KEY` / `WORKER_KEY_PASSWORD` as in [README](README.md).
5. **RPC:** `[network] bootnodes` or `NAKHARAX_RPC_URL` must point to a node of the same chain (e.g. Chain ID 86137 for testnet).
