...
or via the NAKHARAX_MARKETPLACE_ADDRESS environment variable.
"""
...
logger = logging.getLogger("nakharax.contract")

ZERO_ADDRESS = "0x0000000000000000000000000000000000000000"

def _abi_path() -> Path:
    """ABI path: NAKHARAX_ABI_PATH env or default job_marketplace.json beside this module."""
    env_path = os.environ.get("NAKHARAX_ABI_PATH", "").strip()
    if env_path and Path(env_path).exists():
        return Path(env_path)
    return Path(__file__).parent / "job_marketplace.json"

MARKETPLACE_ADDRESS = os.environ.get(
    "NAKHARAX_MARKETPLACE_ADDRESS", ZERO_ADDRESS
).strip()

CHAIN_ID = int(os.environ.get("NAKHARAX_CHAIN_ID", "86137"))
...
class ContractManager:
    ...
    def __init__(
        ...
        """
        Args:
            rpc_url: HTTP/HTTPS URL of the Nakharax JSON-RPC endpoint.
            ...
            contract_address: Deployed JobMarketplace address.  Defaults to
                              the ``NAKHARAX_MARKETPLACE_ADDRESS`` env var, or
